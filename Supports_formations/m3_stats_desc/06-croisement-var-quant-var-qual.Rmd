...
```{r agregat, echo = F}
ag <- dat %>% group_by(ZAU) %>%
  summarise(pop_moy = mean(P14_POP, na.rm = T),                                dens_med = median(densite, na.rm = T),                             nb_com = n())
print(ag)
```
...
- La *boîte à moustaches* (fonction `geom_boxplot()`) affiche, pour chaque modalités de la variable qualitative, la distribution de la variable quantitative, grâce à une boîte à moustaches (la barre la plus basse de la boîte indique Q1, le trait au milieu indique la médiane, et la barre supérieur de la boîte indique Q3).
...
- Le *violinplot* (fonction `geom_violin()`) fonctionne sur le même principe. Les boîtes à moustaches sont remplacés par des graphiques en *violon*, qui représentent la densité de distribution de la variables quantitatives
