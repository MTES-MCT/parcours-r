...
Nous allons voir comment animer notre premier graphique sur la répartition des pays en fonction du taux de mortalité et du PIB, en regardant comment celui ci évolu avec les années.
...
Le graphique précédent est bien sur illisible, un même pays vois ces données affichés pour toutes les années.
...
```{r gganimate2,,warning = FALSE,message = FALSE,error=FALSE}
...
***Transition_states()*** peut s'assimiler à la création de facette, mais qui vont être ensuite utilisée pour créer une animation. Les options permettant de paramétrer cette animation:
...
```{r gganimate3,warning = FALSE,message = FALSE,error=FALSE}
...
La fonction ***anime_save()*** permet de sauvegarder nos animations.
Par défaut elle sauvegarde la dernière animation.

```{r gganimate5,warning = FALSE,message = FALSE,error=FALSE}
anim_save("ma_super_animation.gif")
```
...
Produire le nuage de point du croisement de la part des logements ayant une étiquette A,B,C en fonction de la part du parc récent.
