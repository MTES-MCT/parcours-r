...
Ci dessous un exemple, on reprend les données utilisé pour le diagramme barre.
```{r chap14}
library(knitr)
library(kableExtra)
indicateur311 %>%
...
- ***setNames()*** permet de définir les libellées de colonne;

- ***kable("html")*** permet de produire une sortie html du tableau, l'option caption défini le titre;

- ***kable_styling()*** permet de mettre un theme à notre tableau, ici on lui dit qu'on veut un tableau avec des lignes dont les couleurs alterne entre blanc et gris et un effet de surbrillance au passage de la souris;

- ***row_spec()*** permet de définir des attribut particuliers pour des lignes, ici on lui dit qu'on veut une ligne 1 avec du gras, un fond gris et une police de couleur blanche. L'équivalent pour les colonnes est ***column_spec()***;

- ***add_indent()*** permet de définir sur un vecteur de ligne (ici les lignes 2 à 5) une identation.
...
rpls_DEP_R24  <- rpls_aggrege_large %>% 
  ...
  group_by(TypeZone,Zone) %>% 
  summarise(Logements_collectifs=sum(Logements_collectifs),
            Logements_individuels=sum(Logements_individuels)) %>% 
